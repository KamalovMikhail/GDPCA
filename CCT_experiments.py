...
features = scipy.sparse.load_npz('data/CCT/X.npz')
Af = scipy.sparse.load_npz('data/CCT/sAf.npz')
...
def gen_splits(labels: np.ndarray, idx_split_args: Dict[str, int], test) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    train_idx, validation_idx = train_stopping_split(
        known_idx, labels[known_idx], **stopping_split_args
    )
    if test:
        test_idx = unknown_idx
    else:
        test_idx = exclude_idx(known_idx, [train_idx, validation_idx])
